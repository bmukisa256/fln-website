...
const boardMembers = [
  ...
  { image: "/imgs/justice lillian.jfif", name: "Hon Prof. Lady Justice Lillian Tibatemwa", position: "Chairperson", description: "Justice of the Supreme Court." },
  ...
  { image: "/imgs/joanita.jpg", name: "Joanita Bushara", position: "Member", description: "Managing Partner of Justitia Advocates." }
];
...
// Consistent Profile Image Component
const ProfileImage = ({
  src,
  alt,
  size = "md",
...
const FeatureImage = ({
  ...
  return (
    <div className={`relative rounded-3xl overflow-hidden bg-slate-200 ${className}`}>
      <Image
        src={src}
        alt={alt}
        fill
            ...
            <div className="hidden md:flex items-center gap-1">
              {navLinks.map((item) => (
                <Link
                  ...
                  className="px-5 py-2 text-sm font-medium text-slate-600 hover:text-navy hover:bg-slate-100 rounded-full transition-all"
                >
                  {item}
                </Link>
              ))}
              ...
              <div className="container mx-auto px-6 py-6 flex flex-col gap-2">
                {navLinks.map((item, i) => (
                  <motion.div key={item} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}>
                    <Link
                      href={`/${item.toLowerCase()}`}
                      className="block py-3 px-4 text-lg font-medium text-navy hover:bg-slate-50 rounded-xl transition-colors"
